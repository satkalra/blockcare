...
const GetAllWalletTransactions: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      try {
        ...
        const response = await fetch(`https://gwu-aditya.engine-usw2.thirdweb.com/backend-wallet/sepolia/${walletAddress}/get-all-transactions`, {
          method: 'GET',
          headers: {
  ...
  return (
    <div>
      <h2>Transactions for Wallet: {walletAddress}</h2>
      <ul>
        {transactions && transactions.length > 0 ? (
            <ul>
                ...
                <li key={index}>
                    ...
                    Date: {transaction.minedAt}, Sent to: {transaction.toAddress}
                </li>
                ))}
            </ul>
            ) : (
...
export default GetAllWalletTransactions;
